    ...
    <div class="con">
      <van-cell-group>
        ...
        <van-cell
          ...
          :value="company"
          @click="show = true"
          v-if="detail_id == null||detail_id == undefined"
        />
        <!-- 选择报事的企业 -->
...
export default {
  data() {
    return {
      ...
      companyId: localStorage.getItem('companyId'), //报事企业ID
      name: '阿萨德',
      baoShiType: [],
  ...
  mounted() {
    ...
    this.allUsers()
    //查询当前登录人下面所有企业/部门
    if (this.detail_id == null || this.detail_id == undefined) {
      this.department()
    }
    setTimeout(() => {
      // 事件类型List
  ...
  methods: {
    ...
    //提交报事
      ...
      if (this.company == '') {
        ...
        this.axios
          ...
          .then(data => {
            this.$toast('报事成功')
            this.baoShi(this.$route.query.detail_id)
    ...
    baoShi(obj) {
      this.keepAxios
        .get(this.baseUrl.lockage + '/rest/checkTask/reportEventAfter', {

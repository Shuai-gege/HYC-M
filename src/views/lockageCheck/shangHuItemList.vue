        ...
        <Mescroll-vue ref="mescroll" :up="mescrollUp" @init="mescrollInit" class="mescroll">
          ...
          <van-tab title="进行中">
            ...
            <div
              ...
              :key="i"
              @click="sign(item.id,item.endTime,item.detail_id)"
            >
              <div class="flex col">
                <span>事件类型：</span>
                <span>商户闭店自检</span>
          ...
          <van-tab title="未开始">
            ...
            <div
              ...
              v-for="(item,i) in list"
              :key="i"
              @click="sign1(item.id,item.endTime,item.detail_id)"
            >
              <div class="flex col">
                <span>事件类型：</span>
                <span>商户闭店自检</span>
          ...
          <van-tab title="已结束">
            ...
            <div
              ...
              v-for="(item,i) in list"
              :key="i"
              @click="sign2(item.id,item.endTime,item.detail_id)"
            >
              <div class="flex col">
                <span>事件类型：</span>
                <span>商户闭店自检</span>
              ...
              <div class="flex col">
                <div>
                  <span>自检状态：</span>
                  <span v-if="dangQianTime>new Date(item.end_time)&&item.shop_status ==0">已逾期</span>
                  <span v-else>{{item.ziJian_status}}</span>
                </div>
                <div>
                  <span>自检结果：</span>
                  <span>{{item.ziJian_check}}</span>
...
export default {
  ...
  methods: {
    ...
    getList(page, mescroll, type) {
      this.keepAxios
        .post(this.baseUrl.lockage + '/rest/checkTask/getDetailsByParam', {
          ...
          pageSize: page.size,
          spot_status: type,
          shop_tel: localStorage.getItem('mobile'),
    ...
    upCallback(page, mescroll) {
      if (this.active == 0) {
        this.getList(page, mescroll, '7')
      } else if (this.active == 1) {
        this.getList(page, mescroll, '0')
      } else if (this.active == 2) {
        this.getList(page, mescroll, '8')
